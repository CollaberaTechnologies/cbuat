...
namespace WebApplication6.Controllers
{
    ...
    {
        IHD_DBEntities12 db = new IHD_DBEntities12();
       ...
       [HttpGet]
        ...
        public ActionResult ResetPassword(ResetPasswordModel model)
        {
            ...
            else
            {
                using (IHD_DBEntities12 db = new IHD_DBEntities12())
                {
                    var user = db.CL_USERS.Where(a => a.RID == model.id).FirstOrDefault();
                    if (user != null)
                    {
        ...
        [NonAction]
        public void SendVerificationLinkEmail(string emailID, long rid, string emailFor = "VerifyAccount")
        {
            try
            {
        ...
        public ActionResult ForgotPassword1(string email)
        {
            ...
            using (IHD_DBEntities12 db = new IHD_DBEntities12())
            {
                var account = db.CL_USERS.Where(a => a.EmailID == email).FirstOrDefault();
                if (account != null)
                {
        ...
        public ActionResult ResetPassword(long id = 0)
        {
            ...
            if (id == 0)
            {
                id = (long)Session["uiid"];
            }
            using (IHD_DBEntities12 db = new IHD_DBEntities12())
            {
                var user = db.CL_USERS.Where(a => a.RID == id).FirstOrDefault();
                if (user != null)
                {
    ...
    internal class WebSecurity
    {
        internal static bool UserExists(object userName)
        {
